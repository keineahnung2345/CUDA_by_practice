...
__global__ void histo_kernel( Vector<unsigned int> d_a, Vector<int> d_histo ) {
    ...
    // -:YOUR CODE HERE:-  
    //same shape of a block
    __shared__ int cache[SIZE_H];

    //index inside a grid
    int ith = threadIdx.x + blockIdx.x * blockDim.x;
    int stride = blockDim.x * gridDim.x;
    //gi: index across the whole array
    for(int gi = ith; gi < SIZE; gi += stride){
        atomicAdd(&cache[d_a.getElement(gi)], 1);
    }
    __syncthreads();
//    if(blockIdx.x == 0){
        d_histo.setElement(threadIdx.x, cache[threadIdx.x]);
//    }
}
